...
tally_components <- cleaned %>% components()

tally_components <- tally_components$membership %>% 
  table() %>% 
  as.data.frame() %>% 
  arrange(desc(Freq))
component_id <- tally_components[1,1] %>% as.integer()


# Add the component id's as an edge attribute?
# then filter to only some edge attributes?
